...
export default async function ProjectPage({ params }: Props) {
  ...
  return <main className="detail-page"><Link href="/#projects">← Back to projects</Link><p className="eyebrow">Case study</p><h1>{project.title}</h1><p className="detail-lead">{project.summary}</p><div className="detail-grid"><div><h2>The work</h2><p>{project.description}</p></div><aside><span>Outcome</span><strong>{project.outcome}</strong><div className="tag-list">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></aside></div></main>;
}
